...
argument-hint: '<thread-id> [--background] [--write|--full|--sandbox <mode>] [--worktree|--worktree-name <name>] [--goal <objective>] [--model <luna|sol|astra>] [--effort <none|low|medium|high>] [prompt]'
...
- Accepted efforts are `none`, `low`, `medium`, and `high`; the general policy ceiling is `high`, while `gpt-6-astra` accepts only `low` or `medium` and receives `medium` when no effort is given.
